...
import xml from "xml";
import { arrayify, createAttr, createMetadata, createSnsLambdaEvent, createSnsTopicEvent, parseMessageAttributes, parseAttributes, createMessageId, validatePhoneNumber, topicArnFromName, formatMessageAttributes, } from "./helpers.js";
...
export class SNSServer {
    ...
    publish(topicArn, subject, message, messageStructure, messageAttributes, messageGroupId) {
        ...
        Promise.all(this.subscriptions
            ...
            if (protocol === "sqs") {
                return this.publishSqs(JSON.stringify(createSnsLambdaEvent(topicArn, sub.SubscriptionArn, subject, message, messageId, messageAttributes, messageGroupId)), sub, messageAttributes, messageGroupId);
            }
